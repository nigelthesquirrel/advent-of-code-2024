package com.andystubbs.adventofcode2024.day10.part2
...
fun main() {
    ...
    val input = readInput("/day10/part2/input.txt")
    ...
    for (start in starts) {
        val peakRoutes = mutableListOf<Location>()
        calculateNumberOfPeakRoutes(start, peakRoutes, grid)
        total += peakRoutes.size
    }
    println(total)
}

fun calculateNumberOfPeakRoutes(location: Location, peaks: MutableList<Location>, grid: List<List<Char>>) {

    for (movement in MOVEMENTS) {
        if (canMove(location, movement, grid)) {
            val newLocation = applyMove(location, movement)
            calculateNumberOfPeakRoutes(newLocation, peaks, grid)
        }
    }
